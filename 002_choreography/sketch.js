var s = function (p) {
  ...
  p.draw = function () {
    ...
    p.push();
    let texts = ['SASG', 'Workshop', '002'];
    p.translate(p.width / 2, p.height / 2 - 230);
    for(let j = 0; j < texts.length; j++) {
      p.stroke(255, 50);
      let grp = rfontc.toGroup(texts[j]);
      let rpoints = grp.getPoints();
      for (let i = 0; i < rpoints.length - 1; i++) {
        let rot = 0;//Math.pow(Math.sin(t * Math.PI * 0.5 - i * 0.001) * 0.5 + 0.5, 8.0) * Math.PI * 0.125;
    ...
    p.text("SASG Workshop 002", 100, 100);
    p.textFont(font, 30);
    p.text("Haptics and Generative choreography", 100, 150);
    p.text("by Naoto Hieda & Evelyne Drouin", 100, 200);
    ...
    p.text("Maximum 10 people", 100, 450);
    p.text("dancers and media artists are welcome", 100, 500);
    p.text("Bring comfortable clothes to move", 100, 550);

    // p.text("Robotics soundscape using motors and solenoids, etc.", 100, 950);
    p.text("August 11 (Sat), 2018", 100, 850);
    p.text("13:00 - 16:00", 100, 900);
    p.text("More information: http://naotohieda.com/sasg/w002", 100, 950);
